...
import static org.openhab.binding.tapocamera.internal.TapoCameraChannels.*;
...
public class TapoCameraHandler extends BaseThingHandler implements DynamicStateDescriptionProvider {
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        if (CHANNEL_MANUAL_ALARM.getName().equals(channelUID.getId())) {
            ...
            if (command instanceof OnOffType) {
                ...
                if (cameraState.getMotionDetection().enhanced == null
                        || cameraState.getMotionDetection().enhanced.isEmpty()) {
    ...
    private void getSupportedFeatures(ModuleSpec moduleSpec) {
        ...
        cameraState.setHasLineCrossingDetection(
                moduleSpec.linecrossing_detection != null && moduleSpec.linecrossing_detection == 1);
        cameraState.setHasIntrusionDetection(
                moduleSpec.intrusion_detection != null && moduleSpec.intrusion_detection == 1);
        cameraState.setHasAudioExceptionDetection(
                moduleSpec.audioexception_detection != null && moduleSpec.audioexception_detection == 1);
    }
    ...
    private void setThingProperties(DeviceInfo device) {
        ...
        if (device.full.sensor != null)
            properties.put("Sensor:", device.full.sensor);
        if (device.full.lensName != null)
    ...
    private void updateThingProperties(NetworkInfo network) {
        ...
        if (network.connectionType.ssid != null)
            updateProperty("SSID:", network.connectionType.ssid);
        if (network.connectionType.rssi != null)
            updateProperty("RSSI:", String.valueOf(network.connectionType.rssi));
    }
    ...
    private void processAllResults(ApiMethodResult object) {
        ...
        Optional<ApiMethodTypes> apipMethod = Arrays.asList(ApiMethodTypes.values()).stream()
                .filter(m -> object.method.equals(m.getMethod()) && object.result.has(m.getModule())
        ...
        // detections
            ...
            updateState(CHANNEL_MOTION_DETECTION_ENABLED.getName(),
                    OnOffType.from(motionDetection.enabled.toUpperCase()));
            ...
            if (motionDetection.enhanced != null) {
                ...
                updateState(CHANNEL_PERSON_DETECTION_ENABLED.getName(),
                        OnOffType.from(motionDetection.enhanced.toUpperCase()));
            }
            ...
            updateState(CHANNEL_PERSON_DETECTION_ENABLED.getName(),
                    OnOffType.from(personDetectionInfo.enabled.toUpperCase()));
            ...
            updateState(CHANNEL_INTRUSION_DETECTION_ENABLED.getName(),
                    OnOffType.from(intrusionDetection.enabled.toUpperCase()));
            ...
            updateState(CHANNEL_TARGET_TRACK_ENABLED.getName(),
                    OnOffType.from(targetAutoTrackInfo.enabled.toUpperCase()));
        }
        // image info
            ...
            updateState(CHANNEL_IMAGE_LENS_CORRECTION.getName(),
                    OnOffType.from(((ImageSwitch) data).lensDistortionCorrection.toUpperCase()));
